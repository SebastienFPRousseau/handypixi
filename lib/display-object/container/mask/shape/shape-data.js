...
class ShapeData {
  ...
  constructor(options, fill, nativeLines, shape) {
    ...
    if (
      !(
        shape instanceof PIXI.Circle ||
        shape instanceof PIXI.Ellipse ||
        shape instanceof PIXI.Polygon ||
        shape instanceof PIXI.Rectangle ||
        shape instanceof PIXI.RoundedRectangle ||
        shape === null
      )
    )
    ...
    if (options instanceof PIXI.GraphicsData) {
      ...
      this._out = new PIXI.GraphicsData(
        options.lineWidth,
        options.lineColor,
        options.lineAlpha,
        options.fillColor,
