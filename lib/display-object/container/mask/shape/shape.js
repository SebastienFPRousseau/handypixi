...
const { ShapeData } = require("./shape-data");
...
class Shape extends Mask {
  ...
  set lineColor(color) {
    if (
      !(
        typeof color === "string" &&
        {}.toString.call(color) === "[object String]"
      )
    ) {
   ...
   * lineAlignment
   * @getter
   * This function is a getter for the member lineAlignment.
   * @return {Number} The alignment of any lines drawn (0.5 = middle, 1 = outter, 0 = inner).
   */
  get lineAlignment() {
    return this._out.lineAlignment;
  }

  /**
   * lineAlignment
   * @setter
   * This function is a setter for the member lineAlignment.
   * @param {Number}  value  The alignment of any lines drawn (0.5 = middle, 1 = outter, 0 = inner).
   */
  ...
  set options(options) {
    ...
    if (options.lineAlignment !== undefined) {
      if ({}.toString.call(options.lineAlignment) !== "[object Number]") {
        throw new TypeError("options.lineAlignment must be a number.");
      }

      this._options.lineAlignment = options.lineAlignment;
    }
  }
   ...
   */
  beginFill(
    color = this.options.fillColor || 0,
    alpha = this.options.fillAlpha || 1
  ) {
    ...
    this.lineStyle({
      ...
      alignment: this._options.lineAlignment
    });
  ...
  bezierCurveTo(points) {
    if (
      !(
        ...
        {}.toString.call(points) === "[object Object]"
      )
    ) {
      throw new TypeError("points must be an object.");
    }

    if ({}.toString.call(points.fromX) !== "[object Number]") {
      throw new TypeError("points.fromX must exist and be a number.");
    }

    if ({}.toString.call(points.fromY) !== "[object Number]") {
      throw new TypeError("points.fromY must exist and be a number.");
    }
    ...
    this._out.moveTo(points.fromX, points.fromY);
    this._out.bezierCurveTo(
      points.cpX1,
      points.cpY1,
      points.cpX2,
      points.cpY2,
  ...
  lineStyle(style) {
    ...
    this._out.lineStyle(style.lineWidth, style.color, style.alpha, style.alignment);
  ...
  /**
   * moveTo
   * This function is used in order to move to a given point.
   * @param {Point|Object} point  The point to reach, must contains x and y coordinates.
   * @return {Shape} This Shape object. Good for chaining method calls.
   */
  moveTo(point) {
    if (!(point instanceof Point) 
      && !(typeof point === "object" &&{}.toString.call(points) === "[object Object]")) {
      throw new TypeError("point must be a Point.");
    }

    if ({}.toString.call(point.x) !== "[object Number]") {
      throw new TypeError("point.x must exist and be a number.");
    }

    if ({}.toString.call(point.y) !== "[object Number]") {
      throw new TypeError("point.y must exist and be a number.");
    }

    this._out.moveTo(point.x, point.y);
  ...
  /**
   * drawStar
   * This function is used in order to draw a star into this Shape.
   ...
   * @param {Object}  options  The drawing options for the shape of this star. Like innerRadius and rotation.
   * @return {Shape} This Shape object. Good for chaining method calls.
   */
  drawStar(x, y, pointsNumber, radius, options = {}) {
    if ({}.toString.call(x) !== "[object Number]") {
      throw new TypeError("x must be a number.");
    }
