...
class AnimatedSprite extends Sprite {
  ...
  constructor(autoUpdate = true) {
    ...
    if (autoUpdate instanceof PIXI.extras.AnimatedSprite) {
      ...
      this._out = new PIXI.extras.AnimatedSprite(
        [PIXI.Texture.EMPTY],
        autoUpdate
      );
    }
  ...
  /**
   * onLoop
   * This function is used in order to call a method when 'loop' is true, and an AnimatedSprite is played and loops around to start again.
   * @param {Function} onLoop Function to call.
   */
  onLoop(onLoop) {
    if ({}.toString.call(onLoop) !== "[object Function]") {
      throw new TypeError("onLoop must be a function.");
    }

    this._out.onLoop = onLoop;
  }
  ...
  /**
   * updateAnchor
   * This function is used in order to automatically update anchor to defaultAnchor when frame changes.
   * Changing anchor for each frame allows to pin sprite origin to certain moving feature of the frame
   * Enabling this will override any previously set `anchor` on each frame change
   * @param {Boolean}  value  Automatically update anchor to defaultAnchor or not.
   */
  updateAnchor(value = true) {
    if ({}.toString.call(value) !== "[object Boolean]") {
      throw new TypeError("value must be a boolean.");
    }

    this._out.updateAnchor = value;
  }
}
