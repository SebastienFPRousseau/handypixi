...
class Mesh extends Container {
   ...
   * autoUpdate
   * @getter
   * This function is a getter for the member autoUpdate.
   * @return {Boolean} The uploading verticies behavior.
   */
  get autoUpdate() {
    return this._out.autoUpdate;
  }

  /**
   * autoUpdate
   * @setter
   * This function is a setter for the member autoUpdate.
   * @param {Boolean}  autoUpdate  For backwards compatibility the default is to re-upload verticies each render call. Set this to false and increase vertexDirty to manually re-upload the buffer.
   */
  set autoUpdate(autoUpdate) {
    ...
    this._out.autoUpdate = autoUpdate;
  }
  ...
  set pluginName(plugin) {
    if (
      !(
        ...
        {}.toString.call(plugin) === "[object String]"
      )
    ) {
   ...
   * vertexDirty
   * @getter
   * This function is a getter for the member vertexDirty.
   * @return {Number} Version of mesh verticies array.
   */
  get vertexDirty() {
    return this._out.vertexDirty;
  }

  /**
   * vertexDirty
   * @setter
   * This function is a setter for the member vertexDirty.
   * @param {Number}  vertexDirty  Version of mesh verticies array.
   */
  set vertexDirty(vertexDirty) {
    if ({}.toString.call(vertexDirty) !== "[object Number]") {
      throw new TypeError("vertexDirty must be a number.");
    }

    this._out.vertexDirty = vertexDirty;
  }
  ...
  /**
   * refresh
   * This function is used in order to refresh uvs for generated meshes (rope, plane) sometimes refreshes vertices too.
   * @param {Boolean}  forceUpdate  If true, matrices will be updated any case
   */
  refresh(forceUpdate) {
    this._out.refresh(forceUpdate);
  }
}
