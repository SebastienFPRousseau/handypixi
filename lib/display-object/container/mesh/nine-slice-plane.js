...
class NineSlicePlane extends Plane {
  ...
  constructor(barsSizes = {}) {
    ...
    if (barsSizes instanceof PIXI.mesh.NineSlicePlane) {
      ...
      if (
        !(
          typeof barsSizes === "object" &&
          {}.toString.call(barsSizes) === "[object Object]"
        )
      )
        throw new TypeError("barsSizes must be an object.");

      this._out = new PIXI.mesh.NineSlicePlane(
        PIXI.Texture.EMPTY,
        barsSizes.leftWidth,
        barsSizes.topHeight,
        barsSizes.rightWidth,
        barsSizes.bottomHeight
      );
    }
  }
  ...
  get barsSizes() {
    return {
      leftWidth: this._out.leftWidth,
      topHeight: this._out.topHeight,
      rightWidth: this._out.rightWidth,
      bottomHeight: this._out.bottomHeight,
    };
  }
  ...
  set barsSizes(barsSizes) {
    ...
    if (barsSizes.leftWidth !== undefined)
      this._out.leftWidth = barsSizes.leftWidth;

    if (barsSizes.topHeight !== undefined)
      this._out.topHeight = barsSizes.topHeight;

    if (barsSizes.rightWidth !== undefined)
      this._out.rightWidth = barsSizes.rightWidth;

    if (barsSizes.bottomHeight !== undefined)
      this._out.bottomHeight = barsSizes.bottomHeight;
  }
