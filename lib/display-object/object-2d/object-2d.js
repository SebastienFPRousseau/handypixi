...
class Object2D {
  ...
  addLook(look, key = this._looks.length.toString()) {
    ...
    if (!(typeof key === "string" && {}.toString.call(key) === "[object String]")) {
   ...
   */
  changeLook(id) {
    if (!({}.toString.call(id) === "[object Number]" || (typeof id === "string" && {}.toString.call(id) === "[object String]"))) {
   ...
   */
  addLooks(looks) {
    if (!(Array.isArray(looks) || (typeof looks === "object" && {}.toString.call(looks) === "[object Object]"))) {
    ...
    if (Array.isArray(looks))
    {
      looks.forEach( (look, index) => {
        if (!(look instanceof Look)) {
          throw new TypeError(`Can't add the ${index} element, it must be a Look`);
        }
