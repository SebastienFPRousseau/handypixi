...
class Object2D {
  ...
  /**
   * scale
   * This function is used in order to scale this object.
   * @param {Number}  x  Scaling width.
   * @param {Number}  y  Scaling height.
   */
  scale(x, y) {
    if ({}.toString.call(x) !== "[object Number]") {
      throw new TypeError("x must be a number.");
    ...
    this.out.scale.x = this.out.scale.x * x;

    if( y === undefined ) {
      this.out.scale.y = this.out.scale.y * x;
      return ;
    }
    ...
    this.out.scale.y = this.out.scale.y * y; 
  }

  /**
   * getScale
   * This function is used in order to get scale member of this object.
   * @return {Object} Contains x and y values.
   */
  getScale() {
  ...
  /**
   * setScale
   * This function is used in order to set scale member of this object.
   * @param {Number}  x  Scaling width.
   * @param {Number}  y  Scaling height.
   */
  setScale(x, y) {
   ...
   * @param {String}  key  Optionnal key to access look.
   */
  addLook(look, key = this._looks.length.toString()) {
    if (!(look instanceof Look)) {
      throw new TypeError("look must be a Look.");
    }

    if (
      !(typeof key === "string" && {}.toString.call(key) === "[object String]")
    ) {
    ...
    if (type === Container) {
      ...
      if (type === AnimatedSprite) {
        if (this._out.out.textures[0] === PIXI.Texture.EMPTY) {
          this._out.out.textures[0] = look.out.texture;
        } else {
    ...
    this._looks.push({ [key]: look });
  }

  /**
   * changeLook
   * This function is used in order apply a Look on this object.
   * @param {Number|String}  id  The indice/key of the look.
   */
  changeLook(id) {
    if (
      !(
        {}.toString.call(id) === "[object Number]" ||
        (typeof id === "string" && {}.toString.call(id) === "[object String]")
      )
    ) {
      throw new TypeError("id must be a number or a string.");
    }

    if (typeof id === "string" && {}.toString.call(id) === "[object String]") {
      id = this._looks.findIndex(element => Object.keys(element)[0] === id);
    }
    ...
    const type = this._out.constructor;
    const look = Object.values(this._looks[id])[0];
    ...
    const permut = this._looks[id];
    this._looks[id] = this._looks[this._looks.length - 1];
    this._looks[this._looks.length - 1] = permut;
  }

  /**
   * addLooks
   * This function is used in order to add and apply some Looks on this object.
   * @param {Look[]|Object}  looks  The looks to apply on this object.
   */
  addLooks(looks) {
    if (
      !(
        Array.isArray(looks) ||
        (typeof looks === "object" &&
          {}.toString.call(looks) === "[object Object]")
      )
    ) {
      throw new TypeError("looks must be an array or an object.");
    }
    ...
    if (Array.isArray(looks)) {
      looks.forEach((look, index) => {
        if (!(look instanceof Look)) {
          throw new TypeError(
            `Can't add the ${index} element, it must be a Look`
          );
        }
    ...
    Object.keys(looks).forEach(key => {
      ...
      if (!(look instanceof Look)) {
        throw new TypeError(`Can't add the ${key} element, it must be a Look`);
      }

      this.addLook(look, key);
    });
  }
  ...
  getLookAt(id) {
    ...
    return Object.values(this._looks[id])[0];
  }
