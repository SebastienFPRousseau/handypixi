...
class Star extends Shape
{
	 ...
	 */
	constructor(x = 0, y = 0, pointsNumber = 5, radius = 10, options = {}, nativeLines = false)
	{
		...
		this.beginFill();
		this._out.drawStar(x, y, pointsNumber, radius, options.innerRadius, options.rotation);
		this.endFill();
	}
	...
	{
		if ({}.toString.call(pointsNumber) !== "[object Number]" || pointsNumber <= 1)
	...
	redraw()
	{
		this.clear();
		this.beginFill();
		this._out.drawStar(this._properties.x, this._properties.y, this._properties.pointsNumber, this._properties.radius, this._properties.innerRadius, this._properties.rotation);
		this.endFill();
	}
};
