...
class Object2D {
  ...
  */
	set position(coordinates) {
		if (!(typeof coordinates === "object" && {}.toString.call(coordinates) === "[object Object]")) throw new TypeError("coordinates must be an object.");
  ...
  */
	set data(data) {
		if (!(typeof data === "object" && {}.toString.call(data) === "[object Object]")) throw new TypeError("data must be an object.");
  ...
  */
	destroy(options) {
		...
		if (options.destroyLooks) {
			for (let i = 0, l = this._looks.length; i < l; i++) this._looks[i].destroy({ destroyBase: options.destroyLooksBases });
		}
		...
		if (options.destroyChildren) {
			for (let i = 0, l = this._children.length; i < l; i++) this._children[i].destroy(options);
		} else {
			for (let i = 0, l = this._children.length; i < l; i++) this.children[i]._parent = null;
		}
