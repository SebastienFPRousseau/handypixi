...
const { GLSLShader } = require("./glsl-shader");
const { LookMaskShader } = require("./look-mask-shader");
...
class AbstractShaderFactory {
  ...
  constructor() {
    if (this.constructor === AbstractShaderFactory)
      throw new TypeError(
        "Cannot construct Abstract instances like AbstractShaderFactory directly."
      );
  }
  ...
  createFromGLSL(shader) {
    if (
      !(
        ...
        {}.toString.call(shader) === "[object Object]"
      )
    )
