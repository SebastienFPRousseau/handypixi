...
class VideoTexture extends Texture {
  ...
  constructor(source, { scaleMode, crossorigin, autoPlay }) {
    ...
    if (!(typeof source === "string" && {}.toString.call(source) === "[object String]" || source instanceof HTMLVideoElement)) {
      throw new TypeError("source must be a string or a HTMLVideoElement.");
    }

    if (!({}.toString.call(scaleMode) === "[object Number]" || scaleMode === undefined)) {
      throw new TypeError("scaleMode must be a number.");
    }

    if (!({}.toString.call(crossorigin) === "[object Boolean]" || crossorigin === undefined)) {
      throw new TypeError("crossorigin must be a boolean.");
    }

    if (!({}.toString.call(autoPlay) === "[object Boolean]" || autoPlay === undefined)) {
      throw new TypeError("autoPlay must be a boolean.");
    }

    this._out = PIXI.Texture.fromVideo(source, scaleMode, crossorigin, autoPlay);
  }
