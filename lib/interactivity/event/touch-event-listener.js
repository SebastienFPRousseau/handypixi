...
const { PixiTouchEventListener } = require("./pixi-touch-event-listener.js");
...
class TouchEventListener extends EventListener {
  ...
  constructor(obj, code, handle, data = {}) {
    ...
    this._touchStartEventListener = new PixiTouchEventListener(
      obj,
      PixiTouchEventListener.TOUCH_START,
      this.touchStart,
      this
    );

    this._touchEndEventListener = new PixiTouchEventListener(
      obj,
      PixiTouchEventListener.TOUCH_END,
      this.touchEnd,
      this
    );

    this._touchEndOutsideEventListener = new PixiTouchEventListener(
      obj,
      PixiTouchEventListener.TOUCH_ENDOUTSIDE,
      this.touchEnd,
      this
    );

    this._touchMoveEventListener = new PixiTouchEventListener(
      obj,
      PixiTouchEventListener.TOUCH_MOVE,
      this.touchMove,
      this
    );
  }
  ...
  getDataEvent() {
    ...
    if (this._touch.length >= TWO_FINGERS) {
      deltaX =
        Math.abs(this._touch[0].clientX - this._touch[1].clientX) -
        this._calibration.x;
      deltaY =
        Math.abs(this._touch[0].clientY - this._touch[1].clientY) -
        this._calibration.y;
    } else {
  ...
  calibrate() {
    ...
    if (touch.length >= TWO_FINGERS) {
      this._calibration.x = Math.abs(
        touch[0].clientX - touch[touch.length - 1].clientX
      );
      this._calibration.y = Math.abs(
        touch[0].clientY - touch[touch.length - 1].clientY
      );
  ...
  touchEnd(context, event) {
    for (let i = 0, l = context._touch.length; i < l; i++) {
      if (
        event.data.originalEvent.changedTouches[0].identifier ===
        context._touch[i].identifier
      ) {
  ...
  touchMove(context, event) {
    switch (context.type) {
      ...
      case SCROLL:
        ...
        break;
      default:
        throw new TypeError("context type should either be PINCH or SCROLL.");
    }
  }
