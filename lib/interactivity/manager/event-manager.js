...
class EventManager {
  ...
  constructor(renderer) {
    if (
      ...
      throw new TypeError(
        "renderer must be either a PIXI.WebGLRenderer or a PIXI.CanvasRenderer ."
      );
    }
  ...
  set currentCursorMode(mode) {
    if (
      !(
        typeof mode === "string" && {}.toString.call(mode) === "[object String]"
      )
    ) {
  ...
  mapPositionToPoint(point, position) {
    ...
    this._interactionManager.mapPositionToPoint(
      point.out,
      position.x,
      position.y
    );
