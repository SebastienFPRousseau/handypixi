...
class Environment {
  ...
  constructor() {
    if (this.constructor === Environment)
      throw new TypeError(
        "Cannot construct Abstract instances like Environment directly."
      );
  ...
  register(addHook = undefined, uploadHook = undefined) {
    if (
      addHook !== undefined &&
      {}.toString.call(addHook) !== "[object Function]"
    )
      throw new TypeError("addHook must be a function.");

    if (
      uploadHook !== undefined &&
      {}.toString.call(uploadHook) !== "[object Function]"
    )
   ...
   */
  add(objs) {
    if (Array.isArray(objs)) this._stage.addChildren(objs);
    else this._stage.addChild(objs);
  }
