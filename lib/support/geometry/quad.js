...
const {
  WebGLEnvironment,
} = require("../../rendering-system/system/webgl-environment");
const { Bounds } = require("../../display-object/bounds/bounds");
const { Shader } = require("../../graphic/shader/shader");
...
class Quad {
  ...
  set vertices(vertices) {
    if (!(vertices instanceof Float32Array) || vertices.length !== 8)
      throw new TypeError(
        "vertices must be a Float32Array, its length must be eight."
      );
  ...
  set uvs(uvs) {
    if (!(uvs instanceof Float32Array) || uvs.length !== 8)
      throw new TypeError(
        "uvs must be an Float32Array, its length must be eight."
      );
