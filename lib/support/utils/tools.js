...
const { TextStyle } = require("../../graphic/style/text-style");
...
class Tools {
  ...
  static getSvgSize(svg) {
    if (
      !(typeof svg === "string" && {}.toString.call(svg) === "[object String]")
    )
  ...
  static measureFont(font) {
    if (
      !(
        typeof font === "string" && {}.toString.call(font) === "[object String]"
      )
    )
  ...
  static measureText(text, style, wordWrap = undefined, canvas = undefined) {
    ...
    if (
      {}.toString.call(wordWrap) !== "[object Boolean]" &&
      wordWrap !== undefined
    )
  ...
  /**
   * earcut
   * This function is used in order to triangulate a polygon.
   * @param {Array}  vertices  A flat array of vertice coordinates.
   * @param {Array}  holes  An array of hole indices.
   * @param {Number}  dimensions  The number of coordinates per vertice in the input array.
   * @return {Array}  The triangulated polygon.
   */
  static earcut(vertices, holes, dimensions) {
    if ({}.toString.call(vertices) !== "[object Array]")
      throw new TypeError("vertices must be an array.");

    for (let i = 0, l = vertices.length; i < l; i++) {
      if ({}.toString.call(vertices[i]) !== "[object Number]")
        throw new TypeError("vertices must be an array of numbers.");
    }

    if ({}.toString.call(holes) !== "[object Array]")
      throw new TypeError("holes must be an array.");

    for (let i = 0, l = holes.length; i < l; i++) {
      if ({}.toString.call(holes[i]) !== "[object Number]")
        throw new TypeError("holes must be an array of numbers.");
    }

    if ({}.toString.call(dimensions) !== "[object Number]")
      throw new TypeError("dimensions must be a number.");

    return PIXI.utils.earcut(vertices, holes, dimensions);
  }
}
